...
class ackData {
    public :
        ...
        uint32_t idMax; /// id of the selected module (max)
        bool toParent; /// if sent to parent
    ackData(bool tp,uint16_t dst,uint16_t dstMax,uint32_t id):toParent(tp),distance(dst),distanceMax(dstMax),idMax(id) {};
};
...
private:
    ...
    uint32_t currentIdMax; /// id of the elected module
...
public :
	ABCcenterCode(Catoms3DBlock *host):GenericCodeBlock(host) {};
	~ABCcenterCode() {};
...
/** needed to associate code to module                                      **/
	static Catoms3DBlockCode *buildNewBlockCode(Catoms3DBlock *host) {
