...
GlutWindow(NULL,1,px,py,pw,ph,titreTexture) {
	openingLevel=0;
	selectedBlock=NULL;
	buttonOpen = new GlutButton(this,ID_SW_BUTTON_OPEN,5,68,32,32,"donnees/boutons_fg.tga");
	buttonClose = new GlutButton(this,ID_SW_BUTTON_CLOSE,5,26,32,32,"donnees/boutons_fd.tga",false);
}
...
void GlutSlidingMainWindow::glDraw() {
	...
	if (openingLevel) {
		...
		char str[256];
		uint64_t t = Scheduler::getHeureActuelle();
		...
		if (selectedBlock)
		{ GLfloat x,y,z;
		  selectedBlock->getTopPosition(x,y,z);
		  sprintf(str,"Selected Block : %d\nPosition : %.1f,%.1f",selectedBlock->blocID,x,y);
		}
		else sprintf(str, "Selected Block : None\n(use [Ctrl]+click)");
...
void GlutSlidingMainWindow::selectBlock(Bloc *ptrBlock) {
	if (selectedBlock) selectedBlock->toggleHighlight();
	selectedBlock = ptrBlock;
	selectedBlock->toggleHighlight();
}
...
{ cout << id << "mouseFunc Event :" << mx << "," << my << endl;
  if (mx>x && mx<x+w && my>y && my<y+h) {
...
{ unsigned char *image;
  ...
  { cerr << "Erreur lors de l'ouverture de " << titre << endl;
  } else
  { glGenTextures(1,&id);
